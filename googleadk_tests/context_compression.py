"""
Simple ADK Context Debugger / Compression
"""
...
"""
    ...
    llm_request.config.system_instruction = (
        datetime_text + original_instruction
    )
...
async def main():
    ...
    # Create app with events compaction config
    app = App(
        name="simple_debug_app",
        root_agent=agent,
        events_compaction_config=EventsCompactionConfig(
            compaction_interval=3,  # Compress every 3 new invocations
            overlap_size=1          # Include 1 overlapping invocation
        ),
    )
